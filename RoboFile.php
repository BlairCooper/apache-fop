...
{
    private const PROJECT_NAME = 'apache-fop-plugin';
...
//        $collection->addTast(extractAppImage(self::PROJECT_NAME . '-unsigned-appimage-' . $buildVersion . '.zip', 'ApacheFopWindows'));
    ...
    public function setupImage(string $targetDir): TaskInterface
    {
        $task = null;

        $filePattern = ':fop-(\d+\.\d+)-bin.zip:';
        $zipVersion = self::DEFAULT_ZIP_VERSION;
        $zipPath = null;

        $files = glob('fop*.zip');
        if (empty($files)) {
            $task = Robo\Result::error($this, 'No FOP zip files present');
        } else {
            foreach ($files as $file) {
                $matches = [];

                if (preg_match($filePattern, $file, $matches) &&
                    version_compare($matches[1], $zipVersion, 'gt'))
                {
                    $zipPath = $file;
            ...
            if (version_compare($zipVersion, self::DEFAULT_ZIP_VERSION, 'eq')) {
                $task = Robo\Result::error($this, 'Unable to determine FOP zip file version');
            } else {
                $collection = $this->collectionBuilder();
                $tmpPath = $collection->tmpDir();   // Creates the temp folder.

                $collection->addTask(
                    $this->taskFilesystemStack()
                        ->mkdir($targetDir)
                        ->remove($tmpPath)  // taskExtract will extract the file to a folder and then "rename" it to the target folder. Fails if folder already exists.
                );
                $collection->addTask(
                    $this->taskExtract($zipPath)
                        ->to($tmpPath)
                ...
                $collection->addTask(
                    $this->taskFlattenDir(
                        [
                            "$tmpPath/fop/build/*.jar",
                            "$tmpPath/fop/lib/*.jar",
                            "$tmpPath/fop/conf/fop.xconf",
                            "$tmpPath/LICENSE",
                            "$tmpPath/NOTICE"
                        ]
                        )
